...
module.exports = (bot) => {
  bot.command('registerFortJonesy', async (ctx) => {
    try {
      console.log('[DEBUG] Comando /registerFortJonesy iniciado', { userId: ctx.from?.id });
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        if (!Array.isArray(jsonData)) {
          console.error('[ERROR] fortjonesy.json não é um array válido');
          return ctx.reply('❌ Erro: O arquivo de dados está corrompido.');
        }
        console.log('[DEBUG] Arquivo fortjonesy.json carregado, total de imagens:', jsonData.length);
      }
    } catch (readError) {
      console.error('[ERROR] Falha ao ler fortjonesy.json:', readError.message);
    ...
    try {
      ...
      console.error('[ERROR] Falha ao salvar fortjonesy.json:', writeError.message);
      return ctx.reply('❌ Erro ao salvar a imagem no arquivo.');
    }

    ctx.reply(`Imagem registrada com sucesso no arquivo fortjonesy.json com ID ${usedId}!`);
    ...
    if (logGroup.status) {
      ...
      console.error('[ERROR] Erro no comando /registerFortJonesy:', error.message);
