...
const filePath = path.join(__dirname, '../../database/fortme.json');
...
const extractNameFromCaption = (caption) => {
  ...
  const nameMatch = caption.match(/Nome:\s*(.+)/i);
...
module.exports = (bot) => {
  bot.command('registerFortMe', async (ctx) => {
    try {
      console.log('[DEBUG] Comando /registerFortMe iniciado', { userId: ctx.from?.id });
    ...
    try {
      if (fs.existsSync(filePath)) {
        ...
        if (!Array.isArray(jsonData)) {
          console.error('[ERROR] fortme.json não é um array válido');
          return ctx.reply('❌ Erro: O arquivo de dados está corrompido.');
        }
        console.log('[DEBUG] Arquivo fortme.json carregado, total de imagens:', jsonData.length);
      }
    } catch (readError) {
      console.error('[ERROR] Falha ao ler fortme.json:', readError.message);
    ...
    try {
      ...
      console.error('[ERROR] Falha ao salvar fortme.json:', writeError.message);
      return ctx.reply('❌ Erro ao salvar a imagem no arquivo.');
    }

    ctx.reply(`Imagem registrada com sucesso no arquivo fortme.json com ID ${usedId}!`);
    ...
    if (logGroup.status) {
      ...
      console.error('[ERROR] Erro no comando /registerFortMe:', error.message);
