...
module.exports = (bot) => {
  bot.command('registerFortMe', async (ctx) => {
    ...
    try {
      ...
      console.error('Erro ao ler ou inicializar o arquivo fortme.json:', error.message);
    ...
    try {
      ...
      console.error('Erro ao salvar o arquivo fortme.json:', error.message);
      return ctx.reply('Erro ao salvar os dados no arquivo.');
    }

    ctx.reply(`Imagem registrada com sucesso no arquivo fortme.json com ID ${newId}!`);
