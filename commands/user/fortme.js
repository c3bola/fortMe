...
const FEATURE_ID = 1; // ID da feature fortme
...
module.exports = (bot) => {
    ...
    try {
      ...
      if (ctx.chat.type === 'private') {
        ...
        try {
          ...
          console.log('[SUCCESS] /fortme executado no privado', { userId, contentId: content.id_fortme_contents });
      ...
      console.log('[DEBUG] /fortme executado no grupo', { groupId, groupName });
      ...
      try {
        ...
        const sentMessage = await ctx.replyWithPhoto(content.image_id, {
          ...
          reply_markup: {
            inline_keyboard: [
              [
                { text: '❤️', callback_data: `fortme_heart_${userId}` },
                { text: '😡', callback_data: `fortme_hat_${userId}` }
              ]
            ]
          }
        });
  ...
  // Handler para votação fortme
  bot.action(/^fortme_(heart|hat)_(.+)$/, async (ctx) => {
    try {
      ...
      await ctx.editMessageReplyMarkup({
        inline_keyboard: [
          [
            { text: `❤️ ${votes.hearts}`, callback_data: `fortme_heart_${targetUserId}` },
            { text: `😡 ${votes.hats}`, callback_data: `fortme_hat_${targetUserId}` }
          ]
        ]
      });
